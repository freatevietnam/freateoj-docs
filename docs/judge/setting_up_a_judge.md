...
FreateOJ supports installing judges through Docker and a PyPI package. We
...
runtimes we run on FreateOJ. Tier 2 contains some in-between
...
The session below spawns a `tierfreateoj` judge image in the same server as the site server.
...
> **Note:** For first time developers: Both the judge and site can share a common problems folder,
