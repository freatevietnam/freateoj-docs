...
The FreateOJ judge already defines several checkers.
...
`check` can return either a `CheckerResult` object (`from freateoj.result import CheckerResult`), or a boolean (`case_passed_bool`).
